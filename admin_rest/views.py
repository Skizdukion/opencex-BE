...
def permissions(request):
    ...
    if not (user.is_staff and user.is_superuser and user.is_active):
        return JsonResponse(
            {"status": False}, safe=False, status=status.HTTP_403_FORBIDDEN
        )
    ...
    if request.method == "PUT":
        if not group_id or not group_permissions:
            return JsonResponse(
                {"status": False}, safe=False, status=status.HTTP_400_BAD_REQUEST
            )

        group = Group.objects.get(id=group_id)
        if not group:
            return JsonResponse(
                {"status": False, "error": "Group not found"},
                safe=False,
    ...
    all_permissions_dict = {
        f'{p.content_type.app_label}/{p.content_type.model}/{p.codename.split("_")[0]}': p.id
...
def me(request):
    ...
    response = JsonResponse(
        {"status": True, "user": user_data}, safe=False, status=status.HTTP_200_OK
    )
    return response
...
def upload_image(request):
    ...
    if not is_valid_image(uploaded_file):
        return JsonResponse(
            {"status": False, "error": "Image is not valid"},
            safe=False,
            status=status.HTTP_400_BAD_REQUEST,
        )
