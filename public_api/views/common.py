...
class SummaryView(NoAuthMixin, ThrottlingViewMixin, APIView):
        ...
        examples=[
            OpenApiExample(
                ...
                value={
                    ...
                    "coins": {
                        "BTC": {"name": "Bitcoin", "withdraw": "ON", "deposit": "ON"},
                        "ETH": {"name": "Ethereum", "withdraw": "ON", "deposit": "ON"},
                    },
                },
            )
        ],
    ...
    def get(self, request):
        ...
        def id_filter(**kwargs):
            return (
                ExecutionResult.objects.filter(**kwargs)
                .values("pair")
                .annotate(id=Max("id"))
        ...
        prices_24h = {
            Pair.get(i["pair"]).code: i["price"]
            for i in ExecutionResult.objects.filter(
        ...
        response = {"data": data, "coins": coins}
...
class MarketsListView(NoAuthMixin, ThrottlingViewMixin, APIView):
    ...
    def get(self, request):
        ...
        r = [
            ...
            if i.code not in PairSettings.get_disabled_pairs()
        ]
        r = [
            {
                "id": i["code"],
...
class OrderBookView(NoAuthMixin, ThrottlingViewMixin, APIView):
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="depth",
                type=int,
                description="Number of entries. 100 by default, max. 500",
            ),
        ],
        examples=[
            OpenApiExample(
    ...
    def get(self, request, pair):
        ...
        pair = pair.replace("_", "-")
        request_data = {"pair": pair, "depth": request.GET.get("depth", 100)}
        ...
        response = {
            ...
            "asks": [
                [i["price"], i["quantity"]] for i in StackView.group(data["sells"])
            ],
            "bids": [
                [i["price"], i["quantity"]]
                for i in StackView.group(data["buys"], reverse=True)
            ],
        }
        return Response(response, status=status.HTTP_200_OK)
...
class TradesView(NoAuthMixin, ThrottlingViewMixin, APIView):
    ...
    def get(self, request, pair):
        ...
        depth = 200
        result = (
            ExecutionResult.objects.filter(pair=pair)
            .select_related("matched_order")
            .only(
                "id",
                "matched_order_id",
                "quantity",
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                "Response Example",
                value={
                    "BTC": {"actual": 73.59451, "orders": 0.40877},
                    "EUR": {"actual": 9821.53659869, "orders": 88.51722622},
                    "USD": {"actual": 100078.73854682, "orders": 6.61468349},
                },
            )
        ],
        responses={200: OpenApiTypes.OBJECT},
...
def render_docs(request):
    ...
    html = markdown.markdown(
        md_content, extensions=["extra", "codehilite", "attr_list", "def_list"]
    )
